...
class ScreenOperatorAccessibilityService : AccessibilityService() {
    ...
    private fun executeTermuxCommand(command: String) {
        ...
        val intent = Intent("com.termux.RUN_COMMAND").apply {
            ...
            putExtra("com.termux.RUN_COMMAND_SESSION_ACTION", 9)
     ...
     */
    private fun performLongClickOnNode(node: AccessibilityNodeInfo): Boolean {
        try {
            // Try to perform long click action
            if (node.isLongClickable) {
                return node.performAction(AccessibilityNodeInfo.ACTION_LONG_CLICK)
            }

            // If the node itself is not long-clickable, try to find a long-clickable parent
            var parent = node.parent
            while (parent != null) {
            ...
            Log.e(TAG, "Error performing long click on node: ${e.message}")
            return false
        }
    }
...
fun pressEnterKey() {
    ...
    try {
        ...
        val x = (screenWidth * 0.96f).toInt()
        val y = (screenHeight * 0.935f).toInt()
...
/**
 * Open an app by app name or package name
 */
fun openApp(appNameOrPackage: String) {
    Log.d(TAG, "Opening app: $appNameOrPackage")

    try {
        // First, try to resolve the app name to a package name
        var packageName = appNameOrPackage
        var appName = appNameOrPackage

        // Check if it's likely an app name (doesn't contain dots)
        if (!appNameOrPackage.contains(".")) {
            // Try to get the package name from the app name
            val resolvedPackageName = appNamePackageMapper.getPackageName(appNameOrPackage)
            if (resolvedPackageName != null) {
                packageName = resolvedPackageName
                appName = appNamePackageMapper.getAppName(packageName)
                Log.d(TAG, "Resolved app name '$appNameOrPackage' to package name: $packageName")
            } else {
                Log.w(TAG, "Could not resolve app name '$appNameOrPackage' to package name, trying as-is")
            }
        } else {
            // It's likely a package name, try to get the app name
            appName = appNamePackageMapper.getAppName(packageName)
        }
...
private fun openAppUsingLaunchIntent(packageName: String, appName: String): Boolean {
    try {
        ...
        // Second try: if no launch intent, try to find ANY launchable activity
        if (launchIntent == null) {
            Log.d(TAG, "No standard launch intent, searching for any launchable activity")

            val packageInfo = packageManager.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES)
            val activities = packageInfo.activities

            if (activities != null && activities.isNotEmpty()) {
                // Try to find a main activity
                for (activity in activities) {
                    if (activity.exported) {
                        Log.d(TAG, "Found exported activity: ${activity.name}")
                        launchIntent = Intent()
                        launchIntent.setClassName(packageName, activity.name)
                        launchIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                        break
                    }
                }
                
                // If still no intent, just use the first activity
                if (launchIntent == null && activities.isNotEmpty()) {
                    Log.d(TAG, "Using first activity: ${activities[0].name}")
                    launchIntent = Intent()
                    launchIntent.setClassName(packageName, activities[0].name)
                    launchIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }
            }
        }
    ...
    private fun captureScreenInformation(): String {
        ...
        elements.forEachIndexed { index, element ->
            ...
            // Add text if available and not seen
            element.text?.toString()?.takeIf { it.isNotEmpty() }?.let {
                if (seenValues.add(it)) {
                    screenInfo.append("\"$it\" ")
                }
            }

            // Add content description if available and not seen
            element.contentDescription?.toString()?.takeIf { it.isNotEmpty() }?.let {
                if (seenValues.add(it)) {
                    screenInfo.append("\"$it\" ")
                }
            }

            // Add inferred name if available and not seen
            val buttonName = getButtonName(element)
            if (buttonName.isNotEmpty()) {
                if (seenValues.add(buttonName)) {
                    screenInfo.append("\"$buttonName\" ")
                }
            }
            ...
            screenInfo.append("(${rect.left}, ${rect.top}, ${rect.right}, ${rect.bottom})")
